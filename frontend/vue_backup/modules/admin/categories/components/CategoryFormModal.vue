          ...
          <div class="sm:flex sm:items-start">
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
              <h3 class="text-xl leading-6 font-bold text-gray-900">
                {{ category?.id ? 'Редактировать категорию' : 'Новая категория' }}
              </h3>
              ...
              <div class="mt-6">
                <form @submit.prevent="submit" class="space-y-6">
                  ...
                  <div>
                    ...
                    <div class="mt-1">
                      <input v-model="form.name" type="text" required class="shadow-sm focus:ring-bakery-500 focus:border-bakery-500 block w-full sm:text-sm border-gray-300 rounded-md py-2 px-3 border bg-white text-gray-900\" placeholder="Хлеб, Сдоба, Напитки" />
                    </div>
                  </div>

                  <!-- Описание -->
                  <div>
                    <label class="block text-sm font-medium text-gray-700">Описание (опционально)</label>
                    <div class="mt-1">
                      <textarea v-model="form.description" rows="3" class="shadow-sm focus:ring-bakery-500 focus:border-bakery-500 block w-full sm:text-sm border-gray-300 rounded-md py-2 px-3 border bg-white text-gray-900\" placeholder="..."></textarea>
                    </div>
                  </div>
...
const submit = () => {
  ...
  loading.value = true;
  emit('save', { id: props.category?.id, data: { ...form }, resolve: () => { loading.value = false; close(); }, reject: () => { loading.value = false; } });
};
</script>
